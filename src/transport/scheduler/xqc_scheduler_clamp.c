...
#include "src/transport/scheduler/xqc_scheduler_clamp.h"
#include "src/transport/scheduler/xqc_scheduler_common.h"
#include "src/transport/xqc_send_ctl.h"


static size_t
xqc_clamp_scheduler_size()
{
    return 0;
}

static void
xqc_clamp_scheduler_init(void *scheduler, xqc_log_t *log, xqc_scheduler_params_t *param)
{
    return;
}
...
xqc_path_ctx_t *
xqc_clamp_scheduler_get_path(void *scheduler,
    ...
    xqc_bool_t *cc_blocked)
{
    ...
    xqc_bool_t path_can_send, path_in_handover;
    ...
    xqc_list_for_each_safe(pos, next, &conn->conn_paths_list) {
        ...
        path_in_handover = xqc_scheduler_path_in_handover(path);

        if (!path_can_send || path_in_handover) {
            goto skip_path;
        }
...
skip_path:
        xqc_log(conn->log, XQC_LOG_DEBUG, 
                ...
                "can_send:%d|in_handover:%d|path_status:%d|path_state:%d|reinj:%d|"
                "pkt_path_id:%ui|best_path:%i|", 
                conn, path->path_id, path_srtt, path_class, path_can_send, path_in_handover,
...
const xqc_scheduler_callback_t xqc_clamp_scheduler_cb = {
    .xqc_scheduler_size             = xqc_clamp_scheduler_size,
    .xqc_scheduler_init             = xqc_clamp_scheduler_init,
    .xqc_scheduler_get_path         = xqc_clamp_scheduler_get_path,
};