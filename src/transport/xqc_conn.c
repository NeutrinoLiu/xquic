...
xqc_conn_schedule_packets_to_paths(xqc_connection_t *conn)
{
    ...
    if ((conn->conn_settings.mp_enable_reinjection & XQC_HANDOVER_AWARE_REINJ) && some_path_in_ho(conn)) {
        xqc_conn_reinject_unack_packets(conn, XQC_HANDOVER_AWARE_REINJ);
    }
